...
for year in [0, 5]:
    Disruption(
        ...
        dataset='PPP_project',
        table_id='true_match',
        table_out='true_match_disruption_Y' + str(year),
        var_id='work_id',
        Y=year,
...
for year in [0, 5]:
    for cat in ['', 'cited by examiner', 'cited by applicant']:
        ...
        table_out = 'true_match_disruption_patent' + cat_type + '_y' + str(year)
        print(table_out)
        Disruption(
            project='long-sonar-470413-q7',
            dataset='PPP_project',
...
for year in [0, 5]:
    Disruption(
        ...
        dataset='PPP_project',
        table_id='MM_PPP',
        table_out='MM_PPP_disruption_Y' + str(year),
        var_id='paperid',
        Y=year,
        var_refs='referenced_works',
        var_year='publication_year',
...
for year in [0, 5]:
    for cat in ['', 'cited by examiner', 'cited by applicant']:
        cat_type = '_' + re.sub(' ', '_', cat) if cat else ''
        table_out = 'MM_PPP_disruption_patent' + cat_type + '_y' + str(year)
        print(table_out)
        Disruption(
            project='long-sonar-470413-q7',
            dataset='PPP_project',
